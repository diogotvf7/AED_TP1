...
bool AddRequest::isPossible() const {
    if (student->isInUc(intended->getUc())) throw Oopsie("Can't add " + intended->getUc()->getUcCode() + ' ' + intended->getClassCode() + " to " + student->getName() + "'s schedule because: " + student->getName() + " already is in this UC.");
    if (student->isInClass(intended)) throw Oopsie("Can't add " + intended->getUc()->getUcCode() + ' ' + intended->getClassCode() + " to " + student->getName() + "'s schedule because: " + student->getName() + " already is in " + intended->getUc()->getUcCode() + ' ' + intended->getClassCode() + '.');
    ...
    if (!student->getStatus() && maxStudents - minStudents >= 4 && intended->countStudents() >= maxStudents - 4) throw Oopsie("Can't add " + intended->getUc()->getUcCode() + ' ' + intended->getClassCode() + " to " + student->getName() + "'s schedule because: " + "Adding " + student->getName() + " to " + intended->getUc()->getUcCode() + ' ' + intended->getClassCode() + " will cause imbalance in " + intended->getUc()->getUcCode() + ".");
