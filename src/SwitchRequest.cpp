...
bool SwitchRequest::isPossible() const {
    if (current->getUc() != intended->getUc()) throw Oopsie("Can't switch " + student->getName() + "'s " +
                                                            current->getUc()->getName() + ' ' + current->getName() + " to " +
                                                            intended->getUc()->getName() + ' ' + intended->getName() + " because: both classes are from different UC's.");
    if (!student->isInClass(current)) throw Oopsie("Can't switch " + student->getName() + "'s " +
                                                   current->getUc()->getName() + ' ' + current->getName() + " to " +
                                                   intended->getUc()->getName() + ' ' + intended->getName() + " because: " + student->getName() + " insn't in " +
                                                   current->getUc()->getName() + ' ' + current->getName() + '.');
    if (student->isInClass(intended)) throw Oopsie("Can't switch " + student->getName() + "'s " +
                                                   current->getUc()->getName() + ' ' + current->getName() + " to " +
                                                   intended->getUc()->getName() + ' ' + intended->getName() + " because: " + student->getName() + " already is in " +
    ...
    if (intended->countStudents() >= 30) throw Oopsie("Can't switch " + student->getName() + "'s " +
                                                      current->getUc()->getName() + ' ' + current->getName() + " to " +
                                                      intended->getUc()->getName() + ' ' + intended->getName() + " because: " +
                                                      intended->getUc()->getName() + ' ' + intended->getName() + " is full.");
    if (!student->getStatus() && current->countStudents() <= intended->countStudents() && maxStudents - minStudents >= 4 && intended->countStudents() >= maxStudents - 4) throw Oopsie("Can't switch " + student->getName() + "'s " +
                                                                                                                                                                                       current->getUc()->getName() + ' ' +
                                                                                                                                                                                               current->getName() + " to " +
                                                                                                                                                                                       intended->getUc()->getName() + ' ' +
                                                                                                                                                                                               intended->getName() + " because: " + "Adding " + student->getName() + " to " +
                                                                                                                                                                                       intended->getUc()->getName() + ' ' +
                                                                                                                                                                                               intended->getName() + " will cause imbalance.");

    for (Slot *slot : intended->getSlots())
        for (Class *c : student->getClasses())
            if (c != current)
                for (Slot *slot2 : c->getSlots())
                    if (slot->overlaps(*slot2))  throw Oopsie("Can't switch " + student->getName() + "'s" +
                                                              current->getUc()->getName() + ' ' + current->getName() + " to " +
